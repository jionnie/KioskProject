...
public class Kiosk {
     ...
     */
    public void start() {
        List<MenuItem> cart = shoppingCart.getCart(); // 장바구니 얻어오기
        ...
        while (true) {
            ...
            if (shoppingCart.getCart().isEmpty()) { // 장바구니가 비어있으면
                if (category < 1 || category > menus.get((menus.size() - 1)).getId()) { // 첫 번째 카테고리 번호와 마지막 카테고리 번호 사이의 값이 아닐 경우
                    System.out.println("알림: 메뉴판에 존재하는 카테고리를 선택해주세요.\n");
                    continue;
                }
                kioskView.showMenuItems(category);
            } else { // 장바구니에 항목이 존재한다면
                if (category >= menus.get(0).getId() && category <= menus.get((menus.size() - 1)).getId()) { // 첫 번째 카테고리 번호와 마지막 카테고리 번호 사이의 값일 경우
                    kioskView.showMenuItems(category);
                } else if (category == menus.get(menus.size() - 1).getId() + 1) { // 마지막 카테고리 다음 번호(Orders) 입력 시
                    ...
                    if (order == 1) { // 1을 입력받으면 주문 완료
                        System.out.printf("주문이 완료되었습니다. 금액은 %d원입니다.", shoppingCart.getAmountToPay());
                        return;
                ...
                } else if (category == menus.get(menus.size() - 1).getId() + 2) {
                    continue;
            ...
            kioskView.confirmAddToCart(category, detailMenu);
